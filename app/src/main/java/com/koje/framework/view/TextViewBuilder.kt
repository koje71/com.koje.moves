...
class TextViewBuilder(override val view: TextView) :
    ...
    var sizeList: List<Int> = mutableListOf()
    ...
    fun setTextAndResize(value: String) {
        if (sizeList.size >= value.length) {
            setTextSizeSP(sizeList[value.length - 1])
        }
        setText(value)
    }

    fun setSizes(value: List<Int>) {
        sizeList = value
    }
